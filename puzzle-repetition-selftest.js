...
// Load the module
const mod = require('./puzzle-repetition.js');
const {
  scheduleReview,
  getDueReviews,
  computeIntervalDays,
...
test('Null-archive: saveReviewSchedule does not throw', () => {
  ...
  const orig = require('./game-archive.js');
...
test('game-archive.js exports savePuzzleReview and getPuzzleReview', () => {
  const ga = require('./game-archive.js');
...
test('Gate-4: puzzle-repetition.js contains no makeMove( calls', () => {
  const code = fs.readFileSync(path.join(__dirname, 'puzzle-repetition.js'), 'utf8');
