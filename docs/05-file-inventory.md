...
- `.github/workflows/ci.yml` — `linux` (node 20/22 `npm run check`), `macos` (node 22 `test:unit`), and `browser` (ubuntu node 22, `needs: [linux]`, installs Chromium via `npx playwright install --with-deps chromium`, runs `npm run test:browser` with state redirected to `${{ runner.temp }}`; on `push`/`pull_request` to `main` + nightly `schedule` + `workflow_dispatch`; on-failure artifact upload wired but a no-op today, since the plain scripts enable no reporter) — see HANDOVER.md §16
