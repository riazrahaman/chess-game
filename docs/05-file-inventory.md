...
- `.github/workflows/ci.yml` — `linux` (node 20/22 `npm run check`), `macos` (node 22 `test:unit`), `json-fallback` (ubuntu node 22, `npm run test:unit` with `CHESS_ARCHIVE_FORCE_JSON=1`, so the game-archive JSON backend is exercised on a sqlite-capable Node), and `browser` (ubuntu node 22, `needs: [linux]`, installs Chromium via `npx playwright install --with-deps chromium`, runs `npm run test:browser` with state redirected to `${{ runner.temp }}`; on `push`/`pull_request` to `main` + nightly `schedule` + `workflow_dispatch`; on-failure artifact upload wired but a no-op today, since the plain scripts enable no reporter) — see HANDOVER.md §16 and §17
...
- `test/*-selftest.js` — one standalone script per area; `wave0`–`wave3` suites cover the audited work; `reachability` and `t0-deadcode` are the structural guards; `g4-undo-request-selftest.js` covers the consent-gated undo (76 assertions); `accounts-isolation-selftest.js` pins the `AccountsManager` JSON-path precedence ladder and the no-shared-store guarantee; `static-url-navigation-selftest.js` covers in-memory static URL SPA navigation (8 assertions); `audit-broken-corners-selftest.js` covers the 6 audit corner cases and Gate 4 invariants (9 assertions); `missed-tactics-bounded-selftest.js` covers the per-request engine budget in `routes-review.js` with a stub analyser (8 tests: cache-free, count cap, wall-clock cap, terminal count-free, null-tolerant misses, max-plies reason)
