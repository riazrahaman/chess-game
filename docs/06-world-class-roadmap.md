...
| B25 | **Four direct-ID archived-game paths skipped the requester scoping the list enforces (IDOR).** `GET /api/games/:id`, `GET /api/games/:id/pgn`, `GET /api/games/:id/missed-tactics` and `POST /api/study {kind:'game', gameId}` each called raw `archive.getGame(id)`, serving ANY archived game to ANY requester (found by an external review handoff at baseline `55239e1`; the list path `handleGetGamesEndpoint` was already scoped). Fix: ONE shared requester-aware resolver `resolveArchivedGameForRequester(req, archive, authDeps, id)` in `server.js`, injected into the review and study route hooks — an owned game is readable/importable only by the matching signed-in account; a different signed-in account or an anonymous requester gets 404; an unowned game is guest scope (anonymous may read/import, signed-in may not reach it by id, consistent with `GET /api/games`); `owner_id`/`ownerId` normalized in one place; missing and not-visible share the identical HTTP 404 `'game not found'` (no enumeration, never 401/403); the PGN `Content-Disposition` filename is set only on allowed requests; missed-tactics resolves BEFORE engine work so a rejected request never starts analysis; rejected Study imports create NO chapter and answer 404 instead of the generic 400. NEW live-HTTP `test/direct-id-authorization-selftest.js` (25 assertions: full matrix for the four operations, no-analysis and no-chapter proofs, scoped-list regressions) wired into `test:unit` + `lint`; About unit-suite count 80 -> 81. | `node test/direct-id-authorization-selftest.js` -> 25 passed, 0 failed (native AND `CHESS_ARCHIVE_FORCE_JSON=1`); study-chapters 17/0; about 13; t0-deadcode 27/0; reachability 49 (KNOWN_DARK 10); wave3-hygiene 87; `npm run lint` 0; positive control reproduces the pre-fix leak (signed-in B / anonymous got 200 on all three GETs and 201 on Study import). | `server.js`, `src/routes-review.js`, `src/routes-study.js`, `test/study-chapters-selftest.js`, `test/direct-id-authorization-selftest.js`, `package.json`, `src/ui-about.js`, `test/about-selftest.js` | server | S | **done (fix-archived-game-idor)** |
...
| B26 | **The Gate 4 self-test crashed after 8 passes and was invisible to `npm run check`.** The vm harness's `window` mock was exactly `{ matchMedia: () => ({ matches: false }) }`, but `src/ui.js` registers pointer listeners at load time (`src/ui.js:1908-1912`, C3 pointer support): `window.addEventListener('pointermove', handlePointerMove, { passive: false })` threw `TypeError: window.addEventListener is not a function` inside `vm.runInContext`, so all 35 behavioral assertions behind the 8 static ones were unreachable. The suite was only `--check`ed in `lint` (syntax-only) and NOT wired into `test:unit` at all. Test fix (no assertion changed, nothing skipped; the only product line touched is the About display stat): (1) the `window` mock gains `addEventListener`/`removeEventListener` over a per-harness `windowListeners` registry (the same pattern p1-scrubber/p2-multipv already use, consistent with MockElement); (2) found by running, a second mock gap: `submitMoveToReferee` auto-claims a seat when none is held (`src/ui.js:798-802`) and that `/api/seat/claim` fetch consumed the harness's pending `/api/move` fetch stub, wedging `commandPending` and failing the Space/retry/premove assertions — `window.sessionStorage`/`window.localStorage` mocks now exist and are seeded with `chess_seat_token_default`/`chess_seat_role_default` so load-time `initSeatAuth` adopts the seat and the auto-claim never fires; (3) WIRING: `node test/gate4-selftest.js` appended to the END of BOTH `test:unit` and `lint` (existing `node --check test/gate4-selftest.js` retained), suite count 81 → 82, About stat moved in both `src/ui-about.js` and `test/about-selftest.js`. Honest defect statement: the crash was pre-existing (8 PASS then exit 1), the mock gap caused it, the wiring is the real exposure fix, and NO assertion was weakened — the suite went from 8-pass-then-crash to 43/0, i.e. 35 assertions that could not even run before are now enforced. Mutation proof on a scratch copy (never the repo): reverting only the window mock to the original `{ matchMedia: ... }` reproduces the exact `TypeError` at `evalmachine.<anonymous>:1909`, exit 1, while the real repo passes 43/0. | `node test/gate4-selftest.js` → 43 passed, 0 failed, exit 0 (previously 8 passes then TypeError, exit 1); mutation proof as above; `node test/about-selftest.js` → 13 (count 82); `node test/t0-deadcode-selftest.js` → 27/0; `node test/reachability-selftest.js` → 49; `node test/wave3-hygiene-selftest.js` → 87; `npm run lint` → 0. Changed: `test/gate4-selftest.js`, `package.json`, `src/ui-about.js`, `test/about-selftest.js`. | `test/gate4-selftest.js`, `package.json`, `src/ui-about.js`, `test/about-selftest.js` | test | S | **done (fix/gate4-selftest-harness)** |
| B27 | **The p1-premove self-test had the identical broken vm harness as Gate 4 (B26) and was referenced by no npm script.** `test/p1-premove-selftest.js`'s `window` mock was exactly `{ matchMedia: () => ({ matches: false }) }`, so loading `src/ui.js` threw `TypeError: window.addEventListener is not a function` at `evalmachine.<anonymous>:1909` (the C3 pointer listeners registered at load, `src/ui.js:1908-1912`), and the suite had zero references in `package.json` (neither `test:unit` nor `lint` mentioned it), so nothing ever failed. Fix (harness-only, ported from the repaired Gate 4 harness; NO assertion added/removed/weakened/skipped): the `window` mock gains `addEventListener`/`removeEventListener` over a per-harness `windowListeners` registry plus `window.sessionStorage`/`window.localStorage` mocks, with `sessionStorage` seeded (`chess_seat_token_default` / `chess_seat_role_default`) so load-time `initSeatAuth` adopts the seat and `submitMoveToReferee`'s auto-claim (`src/ui.js:798-802`) never consumes the pending `/api/move` fetch stub; the context-level `localStorage`/`EventSource` mocks are unchanged. WIRED (the honest wire-vs-document decision — the suite is genuine behavioural multi-premove coverage passing 5/0): `node --check test/p1-premove-selftest.js` appended to `lint` and `node test/p1-premove-selftest.js` to `test:unit`; suite count 82 -> 83 with the About stat moved in both `src/ui-about.js` and `test/about-selftest.js`. Mutation proof on a scratch copy (never the repo): reverting only the window mock reproduces the exact `TypeError` at `evalmachine.<anonymous>:1909`, exit 1, while the repo file (byte-identical, `cmp`) passes 5/0. | `node test/p1-premove-selftest.js` -> 5 passed, 0 failed, exit 0 (previously TypeError, exit 1, unwired); mutation proof as above; `node test/about-selftest.js` -> 13 (count 83); `node test/t0-deadcode-selftest.js` -> 27/0; `node test/reachability-selftest.js` -> 49; `node test/wave3-hygiene-selftest.js` -> 87; `npm run lint` -> 0; `npm run test:unit` -> exit 0, 0 `^FAIL:`, 83 distinct suites. Changed: `test/p1-premove-selftest.js`, `package.json`, `src/ui-about.js`, `test/about-selftest.js`. | `test/p1-premove-selftest.js`, `package.json`, `src/ui-about.js`, `test/about-selftest.js` | test | S | **done (fix/p1-premove-harness)** |
| B28 | **A plain visit rewrote the address bar to `/game/<personal-room>`.** `src/ui.js` `initRoomRouting()` minted a random room into `localStorage.chess_personal_room` then `history.replaceState()`'d the path to `/game/<id>`, so every plain visit showed a room path (`https://chess.riazrahaman.com/game/game-<id>`) and `manifest.webmanifest` `start_url: "/"` never stayed `/`. Fix: keep minting/persisting the private room but stop rewriting the URL; `getCurrentRoomId()` resolves path `/game/<room>` → `?room=` → stored → `'default'` (validated), so `withRoomParam` keeps every API/SSE call scoped; `Copy Room Link` emits `/?room=<id>` (a recipient's `?room=` wins over their own stored room); `src/shell.js` lands `?room=`/`/game/<room>` links on play (bare `/` still home) and `Shell.navigate()` preserves `window.location.search` so a share link survives in-app navigation; `src/ui-analysis.js` prefers `window.getCurrentRoomId()`; `service-worker.js` `CACHE_NAME` v5→v6. `test/static-url-navigation-selftest.js` extended 8→16 tests (8 originals byte-identical). Live Chromium: bare `/` stays `/` and lands home with the private room intact; `/?room=xyz` lands play and keeps the room after navigation. | `node test/static-url-navigation-selftest.js` → 16/0; mutation proof (pushState pathname-only → 15/1 exit 1; revert `?room=` landing → 14/2 exit 1); gates reachability 49, t0-deadcode 27/0, wave3-hygiene 87, p3-multiroom 58/0, about 13, lint 0; `npm run test:unit` exit 0 / 0 FAIL / 83 suites; `npm run test:browser` green under the exact CI env block. Changed: `src/ui.js`, `src/shell.js`, `src/ui-analysis.js`, `service-worker.js`, `test/static-url-navigation-selftest.js`, docs. | `src/ui.js`, `src/shell.js`, `src/ui-analysis.js`, `service-worker.js`, `test/static-url-navigation-selftest.js` | client | S | **done (fix/root-url-personal-room)** |
