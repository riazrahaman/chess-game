...
const B7_TMP_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'chess-t0-draw-'));
...
process.env.CHESS_SOCIAL_DB_PATH = path.join(B7_TMP_DIR, 'social.db');
process.env.CHESS_LEAGUES_DB_PATH = path.join(B7_TMP_DIR, 'leagues.db');
...
async function run() {
  ...
  ref2.state.board = require('../src/rules-engine.js').fenToBoard(fiftyFen);
  ...
  try {
    ...
    const refFlag = require('../src/referee-service.js').getReferee(flagRoom);
