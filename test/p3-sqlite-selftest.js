...
async function runTests() {
  ...
  const gameArchiveMod = require('../src/game-archive.js');
...
1. c4 {English opening start} e6 2. Nf3 d5 3. d4 (3. e3) Nf6 4. Nc3 Be7 5. Bg5 O-O 1-0`;
  ...
  try {
    ...
    const scriptRes = await fetch(`${baseUrl}/src/game-archive.js`);
    assert(scriptRes.status === 200, `GET /src/game-archive.js in ALLOWED_FILES: status 200 (got ${scriptRes.status})`);
    const scriptCt = scriptRes.headers.get('content-type');
    assert(scriptCt && scriptCt.includes('application/javascript'), 'GET /src/game-archive.js: content-type application/javascript');
  ...
  assert(indexHtml.includes('src="src/game-archive.js"'), 'index.html includes game-archive.js script tag');
  ...
  const stockfishMod = require('../src/stockfish-worker.js');
  ...
  const moveReviewMod = require('../src/move-review.js');
