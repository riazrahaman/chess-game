...
function refereeCli(...args) {
  const out = execSync(`node ../src/referee-helper.cjs ${args.join(' ')}`, { cwd: __dirname }).toString();
  return JSON.parse(out);
}

function refereeCliWithEnv(env, ...args) {
  try {
    return JSON.parse(execSync(`node ../src/referee-helper.cjs ${args.join(' ')}`, {
...
refereeCli('reset');
const badPromo = execSync(`node ../src/referee-helper.cjs move e2e4q 2>/dev/null || true`, { cwd: __dirname }).toString();
...
const uiSource = require('fs').readFileSync(require('path').join(__dirname, '..', 'src', 'ui.js'), 'utf8');
...
runC3Transport().then(() => runD1SSETransport()).then(() => {
  ...
  const e8UiSource = require('fs').readFileSync(require('path').join(__dirname, '..', 'src', 'ui.js'), 'utf8');
